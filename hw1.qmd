...
Are the winters getting warmer in Maine? Is snowfall decreasing? Are extreme rainfall events increase in size or frequency? Is the hottest day of the year getting hotter?
...
temp_trend <- ggplot(climate_annual, aes(year)) + 
  geom_line(aes(y = tmax), color = "darkorange2") + 
  geom_line(aes(y = tmin), color = "dodgerblue") +
  geom_smooth(aes(y = tmax), method = "lm", color = "darkorange2", se = FALSE) + 
  geom_smooth(aes(y = tmin), method = "lm", color = "dodgerblue", se = FALSE) + 
  labs(y = "Annual Average Temperature (degrees F)", x = "Year") +
...
precip_plot <- ggplot(climate_annual, aes(year)) + 
  ...
  labs(y = "Annual Average Precipitation", 
       x = "Year") + 
...
snow_plot <- ggplot(climate_annual, aes(year)) + 
  ...
  labs(y = "Annual Average Snowfall", 
       x = "Year") + 
...
freezing_plot <- ggplot(freeze_days, aes(year, freeze_days)) + 
  ...
  labs(y="Number of Freezing Days", x="Year") +
...
hot_plot <- ggplot(hottest_day, aes(year, hottest)) + 
  ...
  labs(y="Hottest Day of the Year (degrees F)", x="Year")+
...
flood_plot <- ggplot(flooding_events, aes(year, flood_days)) + 
  ...
  labs(y="Number of Flooding Days (>= 1 inch/day)", x="Year")+
# make breaks for every 10 years on the x axis
   scale_x_continuous(
    breaks = seq(1950, 2020, by = 10),
    labels = seq(1950, 2020, by = 10)
